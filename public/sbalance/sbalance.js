function createSBalance() {
  if (window.SBInterface == null) {
    window.SBInterface = {
      ...
      showMain: function () {},
      readMessage: function (inbound, str_cache) {
        return false;
      },
      ...
      getRememberMe: function () {
        return JSON.stringify(this.m_signin);
      },
  ...
  window.SBalance = {
    ...
    // pre-declear
    getMode: function () {
      return window.SBInterface.getMode();
    },
    ...
    isRememberMe: function () {
      return window.SBInterface.isRememberMe();
    },
    getRememberMe: function () {
      return JSON.parse(window.SBInterface.getRememberMe());
    },
  ...
  if (dmodal == null) {
    ...
    dmodal.$modal.on('hidden.bs.modal', function () {
      ...
      setTimeout(function () {
        ...
        if (dmodal._data == null) dmodal._open();
      }, 0);
    });
...
window.toast = function (options) {
  ...
  if (window.dtoast.toast == null) {
    ...
    function show_toast(xtoast, item) {
      ...
      window.dtoast.toast = new bootstrap.Toast(xtoast[0], {
        delay: 3000
      });
...
window.popver = function (pop_file, param, callback) {
  ...
  function _event_trigger($element, event) {
    $element.each(function (idx, element) {
      ...
      if (fnCallback != null) fnCallback.call(dpopver);
    });
  }
  SBalance.setTimewait(true);
  var dpopver = {
  ...
  $popver[0].addEventListener('mousedown', function (event) {
    ...
    if (mcontent == null || mcontent.length <= 0) return;
    mcontent = mcontent[0];
    ...
    console.log("chrect~", `${oleft} <= ${event.clientX} <= ${oright} : `, `${otop} <= ${event.clientY} <= ${obottom}`);
    var is_in = false;
    if (event.clientX >= oleft && event.clientX <= oright) {
      if (event.clientY >= otop && event.clientY <= obottom) {
        is_in = true;
        console.log("inrect~", `${oleft} <= ${event.clientX} <= ${oright} : `, `${otop} <= ${event.clientY} <= ${obottom}`);
      }
    }
    if (is_in != true) {
      dpopver.close();
