...
function toInteger(number, decimal) {
  ...
  if (decimal != null && parseInt(decimal) > 0) {
    ...
    if (number != null && isNaN(number) == false && number != 0) return number;
    return 0;
  } else {
...
function thousandFormatter(number, decimal) {
  ...
  if (vals[0]) number = vals[0].toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');
  if (vals[1]) number += '.' + vals[1];
  return number;
}
...
function formatterDateSqlToFmt(value) {
  ...
  return ntime.getFullYear() + '-' + ('0' + (ntime.getMonth() + 1)).slice(-2) + '-' + ('0' + ntime.getDate()).slice(-2);
}
...
function amountFormat(unit, amount) {
  ...
  if (typeof amount == 'object') {
    if (amount._val == 0 && amount._cnt > 0) value = "0";else value = moneyFormatter(amount._val);
  } else {
