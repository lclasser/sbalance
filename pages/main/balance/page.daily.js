...
function process_daily(ppage, $list, inbound, cbfinish) {
  ...
  window.SBalance.request("/main/message/daily", inbound).then(function (res) {
    // stracer.log("res:", res);
    var qry_result = {
      sum_count: 0,
    ...
    Object.keys(date_list).reverse().forEach(function (key, idx) {
      ...
      switch (item_date.getDay()) {
        case 0:
          obj_card.find(".daily-week").addClass("week-sunday");
          break;
        case 6:
          obj_card.find(".daily-week").addClass("week-satday");
          break;
        default:
          obj_card.find(".daily-week").addClass("week-norday");
          break;
      }
      ...
      data.items.forEach(function (item, idx) {
        ...
        obj_row.find("#item_time").html(item.sms_time.substring(0, 2) + ":" + item.sms_time.substring(2, 4));
        ...
        if (is_accum == false) {
          ...
          if (sms_amount < 0) {
            switch (item.sms_class) {
              case 'W':
                obj_amount.addClass("cancel-withdraw");
                break;
              case 'D':
                obj_amount.addClass("cancel-deposit");
                break;
              default:
                obj_amount.addClass("text-dark");
                break;
            }
          } else if (item.ci_type == 'T') {
            // Transfer
            switch (item.sms_class) {
              case 'W':
                obj_amount.addClass("transfer-withdraw");
                break;
              case 'D':
                obj_amount.addClass("transfer-deposit");
                break;
              default:
                obj_amount.addClass("text-dark");
                break;
            }
          } else if (item.ci_type == 'E') {
            // Exeption
            switch (item.sms_class) {
              case 'W':
                obj_amount.addClass("except-withdraw");
                break;
              case 'D':
                obj_amount.addClass("except-deposit");
                break;
              default:
                obj_amount.addClass("text-dark");
                break;
            }
          } else if (item.ci_type == 'N' || item.ci_type == null) {
            // Normal
            switch (item.sms_class) {
              case 'W':
                obj_amount.addClass("amount-withdraw");
                break;
              case 'D':
                obj_amount.addClass("amount-deposit");
                break;
              default:
                obj_amount.addClass("text-dark");
                break;
